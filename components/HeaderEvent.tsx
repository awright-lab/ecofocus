...
/**
 * Secondary header with a top countdown banner for SB’25.
 * SB'25 dates: Oct 13–16, 2025 (San Diego). Countdown targets Oct 13, 2025.
 * Adjust TARGET_ISO to your preferred kickoff time if needed.
 */
const EVENT_NAME = "SB’25 San Diego";
const EVENT_URL = "https://sustainablebrands.com/events/sb25";
const TARGET_ISO = "2025-10-13T00:00:00-07:00"; // Oct 13, 2025 00:00 PT (San Diego)
...
function CountdownBanner() {
  const { diff, days, hours, minutes, seconds } = useCountdown(TARGET_ISO);
  const reduceMotion = useReducedMotion();
  const happening = diff === 0;

  return (
    <div
      className="fixed inset-x-0 top-0 z-[95] h-10 bg-neutral-900/95 text-white backdrop-blur supports-[backdrop-filter]:bg-neutral-900/75 border-b border-white/10"
      role="region"
    ...
    >
        ...
        <div className="flex items-center gap-2 text-xs sm:text-sm">
          ...
          {happening ? (
            <span className="font-semibold">
              {EVENT_NAME} is happening now — join us in San Diego.
            </span>
          ) : (
            <>
      ...
      {/* Thin brand accent under the banner */}
      <div className="absolute bottom-0 left-0 h-0.5 w-full bg-gradient-to-r from-emerald-500 via-teal-400 to-blue-500" />
    </div>
  );
}

export default function HeaderEvent() {
  // --- Your existing Header logic below (copied & trimmed to fit under banner) ---
  ...
  return (
    <>
      ...
      {/* Your header, offset under the banner via top-10 */}
      <header
        data-home={isHome ? 'true' : 'false'}
        data-scrolled={isScrolled ? 'true' : 'false'}
        className={`fixed inset-x-0 top-10 z-[90] transition-colors ${
          isScrolled
      ...
      >
        <div className="mx-auto max-w-7xl h-14 md:h-20 px-4 sm:px-6">
          <div className="flex h-full items-center justify-between">
            {/* Logo (space preserved; invisible on home) */}
        ...
        {/* Homepage-only navbar logo behavior (unchanged) */}
        <style jsx global>{`
      ...
      {/* Spacer so page content isn't hidden behind fixed elements */}
      <div className="h-[calc(40px+5rem)] md:h-[calc(40px+6rem)]" />
    </>
  );
}
