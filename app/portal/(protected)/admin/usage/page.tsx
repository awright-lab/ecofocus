  ...
  return (
    <RoleGuard role="support_admin" redirectTarget="/portal/admin/usage">
      {async (access) => {
        ...
        const allowanceUsagePercent = annualHoursLimit ? Math.min(Math.round((hoursUsed / annualHoursLimit) * 100), 999) : 0;
        const seatsPurchased = subscription?.seatsPurchased ?? 0;
        const seatsUsed = subscription?.seatsUsed ?? 0;
        const seatsRemaining = Math.max(seatsPurchased - seatsUsed, 0);
        const seatUsagePercent = seatsPurchased ? Math.min(Math.round((seatsUsed / seatsPurchased) * 100), 999) : 0;
        const allowanceHealthLabel =
          allowanceUsagePercent >= 100 ? "Allowance exhausted" : allowanceUsagePercent >= 85 ? "Approaching limit" : "Healthy";
        const seatHealthLabel =
          seatUsagePercent >= 100 ? "Seat limit reached" : seatUsagePercent >= 85 ? "Nearly full" : "Capacity available";
          ...
          <div className="space-y-6">
            ...
            <section className="grid gap-4 md:grid-cols-4">
              ...
              <div className="rounded-[28px] border border-emerald-200 bg-emerald-50 p-5">
                ...
                <p className="mt-2 text-3xl font-semibold text-emerald-950">{annualHoursLimit}</p>
              ...
              <div className="rounded-[28px] border border-sky-200 bg-sky-50 p-5">
                ...
                <p className="mt-2 text-3xl font-semibold text-sky-950">{hoursUsed}</p>
              ...
              <div className="rounded-[28px] border border-amber-200 bg-amber-50 p-5">
                ...
                <p className="mt-2 text-3xl font-semibold text-amber-950">{seatsRemaining}</p>
            ...
            <section className="grid gap-6 xl:grid-cols-2">
              <div className="rounded-[32px] border border-slate-200 bg-white p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-950">Allowance utilization</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-600">
                      Track how close this workspace is to its annual usage ceiling before you adjust limits.
                    </p>
                  </div>
                  <p className="rounded-full bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700">
                    {allowanceHealthLabel}
                  </p>
                </div>
                <div className="mt-5 rounded-[24px] bg-slate-50 p-5">
                  <div className="flex items-end justify-between gap-4">
                    <div>
                      <p className="text-3xl font-semibold text-slate-950">{allowanceUsagePercent}%</p>
                      <p className="mt-2 text-sm text-slate-600">
                        {hoursUsed} of {annualHoursLimit} hours used
                      </p>
                    </div>
                  ...
                  <div className="mt-4 h-3 overflow-hidden rounded-full bg-white">
                    <div
                      className={`h-full rounded-full ${
                        allowanceUsagePercent >= 100
                          ? "bg-rose-500"
                          : allowanceUsagePercent >= 85
                            ? "bg-amber-500"
                            : "bg-emerald-500"
                      }`}
              ...
              <div className="rounded-[32px] border border-slate-200 bg-white p-6">
                <div className="flex flex-wrap items-start justify-between gap-4">
                  <div>
                    <h3 className="text-lg font-semibold text-slate-950">Seat utilization</h3>
                    <p className="mt-2 text-sm leading-6 text-slate-600">
                      See whether seat capacity is comfortably available or close to needing a subscription change.
                    </p>
                  </div>
                  <p className="rounded-full bg-slate-100 px-4 py-2 text-sm font-medium text-slate-700">
                    {seatHealthLabel}
                ...
                <div className="mt-5 rounded-[24px] bg-slate-50 p-5">
                  <div className="flex items-end justify-between gap-4">
                    <div>
                      <p className="text-3xl font-semibold text-slate-950">{seatUsagePercent}%</p>
                      <p className="mt-2 text-sm text-slate-600">
