...
export default function EcoNuggetInsightsClient({
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-7xl px-4 sm:px-6 py-10 sm:py-12 md:py-16 relative z-10">
        ...
        <Link href={featured.link} aria-label={`Read: ${featured.title}`} className="group block mb-10 sm:mb-12 md:mb-16">
          ...
          >
            ...
            {/* Text: static on mobile, overlay from md+ */}
            <div className="p-6 sm:p-8 md:absolute md:inset-0 md:flex md:flex-col md:justify-end">
              ...
              {featured.excerpt && (
                <p className="mb-4 max-w-2xl text-[15px] sm:text-base line-clamp-4 sm:line-clamp-3 md:line-clamp-none">
                  {featured.excerpt}
                </p>
              )}
              {featured.time && <div className="text-xs text-gray-300">{featured.time}</div>}

              {/* CTA lives in normal flow on mobile; overlay at md+ */}
              <span
      ...
      {/* Decorative orbs – wrapped so they can’t cause horizontal scroll */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
        <div
          className="absolute -left-24 top-10 h-56 w-56 rounded-full bg-emerald-500/20 blur-3xl"
          style={reduceMotion ? undefined : { animation: 'pulse 2.5s ease-in-out infinite' }}
        />
        <div
          className="absolute -right-20 -bottom-20 h-52 w-52 rounded-full bg-blue-500/20 blur-3xl"
