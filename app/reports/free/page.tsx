...
export default function FreeReportsPage() {
  ...
  return (
    <>
      ...
      {/* Controls (white, no hero/gradient) */}
          ...
          <div className="flex flex-col gap-3 md:flex-row md:items-center">
            ...
            <div className="relative w-full md:max-w-sm">
              <input
                ...
                placeholder="Search free reports…"
                aria-label="Search free reports"
        ...
        <section className="mx-auto max-w-7xl px-6 py-8">
          ...
          {view === 'grid' && (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {shown.map((r) => (
                <FreeReportCard key={r.id} r={r} onDownload={() => openDownload(r)} />
              ))}
            </div>
          )}
          ...
          {view === 'list' && (
            <div className="divide-y divide-gray-200 rounded-xl border">
              {shown.map((r) => (
                <FreeReportRow key={r.id} r={r} onDownload={() => openDownload(r)} />
              ))}
            </div>
          )}
          ...
          {filtered.length === 0 && (
            <div className="mt-8 rounded-xl border border-dashed border-gray-300 p-10 text-center text-gray-600">
              No free reports match your filters.
              <div className="mt-4">
                <button
                  onClick={clearAll}
...
/* ================== Cards (free download CTAs) ================== */

function FreeReportCard({ r, onDownload }: { r: ReportCardModel; onDownload: () => void }) {
...
function FreeReportRow({ r, onDownload }: { r: ReportCardModel; onDownload: () => void }) {
  const cover = r.cover || '/images/report-cover-fallback.jpg';
  return (
    <div className="flex gap-4 p-4">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="text-sm font-semibold text-emerald-700 whitespace-nowrap">Free</div>
        </div>
