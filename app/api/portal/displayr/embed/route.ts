...
export async function GET(req: NextRequest) {
  ...
  if (payload.companyId !== access.company.id || payload.userId !== access.user.id) {
    return NextResponse.json({ error: "Embed token does not match this portal session." }, { status: 403, headers: NOINDEX_HEADERS });
  }

  const displayrUrl = await resolveDisplayrEmbedUrl(payload.companyId, payload.dashboardSlug);
  if (!displayrUrl) {
